...
class OmicsFormer(nn.Module):
    def __init__(self, gene_list, enc_mod, enc_hid, enc_layers, post_latent_dim, dec_mod, dec_hid, dec_layers,
                 ...
                 num_clusters=16, dae=True, lamda=0.5, w_kl=0.5, mask_beta=False, post_hoc=True, reg_token=False, reg_token_number=4, projection=False, hidden_drop=0.3, **kwargs):
        ...
        if projection:
            self.project_head = ProjectionHead(enc_hid, 256, 128)
        else:
            self.project_head = None 
        
        if reg_token:
            self.reg_token = nn.Parameter(torch.randn(reg_token_number, enc_hid))
            self.reg_token_number = reg_token_number
        else:
            self.reg_token = None
        ...
        if latent_mod=='vae':
            self.latent.add_layer(type='vae', enc_hid=enc_hid, latent_dim=post_latent_dim, kl_weight=w_kl)
        elif latent_mod=='ae':
            ...
            self.latent.add_layer(type='vqvae', enc_hid=enc_hid, latent_dim=post_latent_dim, num_categories=batch_num)
        ...
        if dsbn:
            self.pre_latent_norm = PreLatentNorm('dsbn', enc_hid, batch_num)
        else:
            self.pre_latent_norm = PreLatentNorm('ln', enc_hid)
        # self.post_latent_norm = nn.LayerNorm(post_latent_dim, dataset_num)
 
        self.post_latent_norm = PreLatentNorm('dsbn', post_latent_dim, batch_num)
        
        self.aug = nn.Dropout(hidden_drop)
        

    def forward(self, x_dict, input_gene_list=None, d_iter=False, return_atten=False):
        ...
        if self.reg_token is not None:
            x_dict['h'] = torch.cat([x_dict['h'], self.reg_token])
        #print(x_dict['h'].shape)
        x_dict['h'] = self.encoder(x_dict, return_atten)['hidden']
        if self.reg_token is not None:
            x_dict['h'] = x_dict['h'][:-self.reg_token_number]
        #x_dict['h'] = self.pre_latent_norm(x_dict)
        x_dict['h'], latent_loss = self.latent(x_dict)
        
        if self.project_head is not None:
            projection = self.project_head(torch.cat([x_dict['h'], self.aug(x_dict['h'])]))
        
        if self.post_hoc:
            x_dict['h'] = self.post_latent_norm(x_dict)
        ...
        if d_iter:
            ...
            if self.project_head is not None:
                out_dict['projection'] = projection
            else:
                out_dict['projection'] = None     
            
            return out_dict, loss

    def get_representation(self, x_dict, input_gene_list=None, d_iter=False, return_atten=True):
        #if self.mask_type == 'input':
            #x_dict = self.mask_model.apply_mask(x_dict)
        x_dict['h'] = self.embedder(x_dict, input_gene_list)
        #if self.mask_type == 'hidden':
        ...
        if self.reg_token is not None:
            x_dict['h'] = torch.cat([x_dict['h'], self.reg_token])
        tmp_dict = self.encoder(x_dict, return_atten)
        x_dict['h'] = tmp_dict['hidden']
        attn = tmp_dict['attn']
        del tmp_dict
        #x_dict['h'] = self.pre_latent_norm(x_dict)
        if self.reg_token is not None:
        ...
        x_dict['h'], latent_loss = self.latent(x_dict)
        return x_dict, attn  
    '''def get_atten(self, x_dict, input_gene_list=None, d_iter=False, return_atten=True):
        if self.mask_type == 'input':
            x_dict = self.mask_model.apply_mask(x_dict)
        x_dict['h'] = self.embedder(x_dict, input_gene_list)
        if self.mask_type == 'hidden':
            x_dict = self.mask_model.apply_mask(x_dict)
        #x_dict['h'] = torch.cat([x_dict['h'], self.reg_token])
        _, weight_1 = model.encoder.layers[1].layer.self_attn()
        _. weight_2 = model.encoder.layers[2].layer.self_attn'''
