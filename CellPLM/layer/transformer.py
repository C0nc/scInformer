...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        x = src
        if self.norm_first:
            atten_output, atten_map = self._sa_block(self.norm1(x), src_mask, src_key_padding_mask)
            x = x + atten_output
            x = x + self._ff_block(self.norm2(x))
        else:
            atten_output, atten_map = self._sa_block(x, src_mask, src_key_padding_mask)
            x = self.norm1(x + atten_output)
            x = self.norm2(x + self._ff_block(x))

        #print(atten_map.shape)
        return x, atten_map.to('cpu')
...
class VanillaTransformerLayer(nn.Module, AbstractTrasnformerLayer):
    ...
    def forward(self, x, attn_mask=None, output_attentions=False):
        ...
        x = x.unsqueeze(1)
        x, atten = self.layer(x, attn_mask)
